...
- **Base:** a **single self-hosted Protomaps PMTiles multi-region extract** covering all seed cities in one file — *not* per-city files. `<MapBase>` points at one Blob URL; no filename resolution by city id. No API key, no runtime third-party call.
  ...
  - **Glyphs are self-hosted under `public/fonts/`**, for the same no-runtime-dependency reason as the tiles: vector tiles carry label text but not the fonts to draw it, and a hosted glyph CDN would reintroduce exactly the third-party call the extract exists to avoid. The style's `glyphs` is a relative URL. Only the stacks and ranges the style actually references are vendored — one Latin stack today, not the full 6MB set. See `tiles/README.md`.
  - **Label styling stays subordinate.** Street and place names exist so the reader can orient; the price-lean polygons remain the subject. Low-contrast ink, small sizes, local place kinds only (never country/region/city, which would duplicate the module's own area markers), and street names only at the deepest zoom.
